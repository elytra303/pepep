...
public class InventoryTask implements IMinecraft {
    ...
    public static void swapAndUse(Item item, String searchName, boolean task) {
        ...
        if (cooldownProgress > 0) {
            String time = MathUtils.round(cooldownProgress, 0.1) + "с";
             return;
        }
    ...
    public static void updateSlots() {
        ...
        ItemStack stack = Registries.ITEM.get(MathUtils.getRandom(0, 100)).getDefaultStack();
        mc.interactionManager.clickSlot(screenHandler.syncId, 0, 0, SlotActionType.PICKUP_ALL, mc.player);
    }
    ...
    public static void selectCompass() {
        ...
        if (slot != null) {
            mc.player.getInventory().setSelectedSlot(slot.id < 9 ? slot.id : 0);
